...
Use a reusable workflow only when multiple repositories require the same
stable, versioned, independently tested job contract. Use a composite action
for a bounded repeated step sequence. Keep repository-specific quality
...
## Shared tooling line support

The locator-selected latest stable `1.x` release is the default for new
adoption and the recommended upgrade target. An existing repository pinned to
an older supported `1.x` release remains valid; advancing the central locator
does not by itself require an immediate consumer pull request.

Consumer owners MAY batch compatible patch and minor upgrades on their normal
maintenance cadence. An immediate upgrade is required only when the selected
release has a security or integrity defect, produces a material false failure,
cannot execute its declared contract, or is explicitly declared unsupported
with a bounded replacement deadline. Release notes MUST identify those cases.
Routine locator movement, regenerated bytes, or fresher evidence alone is not
an emergency upgrade condition.
