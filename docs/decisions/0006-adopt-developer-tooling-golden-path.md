...
governance repository. The shared implementation release selected by the
immutable operational locator is consumed through that locator, and
workflow-template discovery plus the released bootstrap fixture are As-built
in the policy repository. Each participating repository still owns its
metadata, commands, manifests, locks, exceptions, migration, evidence, hosting
adapters, and conformance status.
